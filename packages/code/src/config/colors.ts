type HEX = string
type TailwindColors = Record<string, Record<number, HEX>>
...
function mapColorsToNames(colors: TailwindColors): Record<HEX, string> {
  ...
  for (const colorName in colors) {
    for (const variationNumber in colors[colorName]) {
      const hex = colors[colorName][variationNumber]
      result[hex] = `${colorName}-${variationNumber}`
    }
  }
  return result
}

export const colorMap: InferenceColorMap = mapColorsToNames(colors)
